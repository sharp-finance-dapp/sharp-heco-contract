...
Vyper contracts used in [Sharp](https://www.sharp.finance/) exchange pools.

## Overview

Sharp is an exchange liquidity pool on Ethereum designed for extremely efficient stablecoin trading and low risk, supplemental fee income for liquidity providers, without an opportunity cost.

Sharp allows users to trade between correlated cryptocurrencies with a bespoke low slippage, low fee algorithm. The liquidity pool is also supplied to lending protocol where it generates additional income for liquidity providers.
...
Sharp contracts are compiled using [Vyper](https://github.com/vyperlang/vyper), however installation of the required Vyper versions is handled by Brownie.
...
```bash
git clone https://github.com/sharpfi/sharp-contract.git
cd sharp-contract
...
* New Sharp pools are built from the contract templates at [`contracts/pool-templates`](contracts/pool-templates)
* Once deployed, the contracts for a pool are added to [`contracts/pools`](contracts/pools)

See the documentation within [`contracts`](contracts) and it's subdirectories for more detailed information on how to get started developing on Sharp.

### Running the Tests

The [test suite](tests) contains common tests for all Sharp pools, as well as unique per-pool tests. To run the entire suite:
...
There is also an active [bug bounty](https://www.sharp.finance/bugbounty) for issues which can lead to substantial loss of money, critical bugs such as a broken live-ness condition, or irreversible loss of funds.

## License

(c) Sharp.Finance, 2021 - [All rights reserved](LICENSE).
